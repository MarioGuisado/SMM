...
/**
 * Clase para definir el "smiley" o cara. Extiende Area.
 * @author Mario Guisado García
 */
public class cara extends Area implements figuras{
    /**
    ...
    /**
     * Constructor, crea los objetos necesarios para su formación y realiza las operaciones necesarias para llevar a cabo el resultado.
     * Hace uso de operaciones add y subtract para conseguirlo.
     * @param p objeto Point2D necesario para construir la cara en la posición en la que indique el mismo.
     */
    public cara(Point2D p){
        Ellipse2D cabeza = new Ellipse2D.Double(p.getX(), p.getY(), 100, 100);
        Ellipse2D ojo1 = new Ellipse2D.Double(p.getX()+25, p.getY()+15, 20, 20);
        Ellipse2D ojo2 = new Ellipse2D.Double(p.getX()+55, p.getY()+15, 20, 20);
        QuadCurve2D labio = new QuadCurve2D.Double(p.getX()+10, p.getY()+55, p.getX()+50, p.getY()+90, p.getX()+90, p.getY()+55);
       
        add(new Area(cabeza));
        subtract(new Area(ojo1));
        subtract(new Area(ojo2));
        subtract(new Area(labio));
        
    }
    
    /**
     * Define la localización de la cara en el lienzo. Utiliza el punto pasado como parámetro para trasladar la figura calculando la diferencia entre
     * la posición inicial y la nueva, y haciendo uso del método getTranslateInstance de la clase AffineTransform.
     * @param pos objeto Point2D necesario para trasladar la figura.
     */
    public void setLocation(Point2D pos) {
       AffineTransform at;
       at = AffineTransform.getTranslateInstance(pos.getX() - this.getBounds2D().getX(), pos.getY() - this.getBounds2D().getY());
       transform(at);
    }
    /**
